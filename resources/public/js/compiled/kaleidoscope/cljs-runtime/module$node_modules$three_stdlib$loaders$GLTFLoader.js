...
9987:_three.LinearMipmapLinearFilter},WEBGL_WRAPPINGS={33071:_three.ClampToEdgeWrapping,33648:_three.MirroredRepeatWrapping,10497:_three.RepeatWrapping},WEBGL_TYPE_SIZES={SCALAR:1,VEC2:2,VEC3:3,VEC4:4,MAT2:4,MAT3:9,MAT4:16},ATTRIBUTES={POSITION:"position",NORMAL:"normal",TANGENT:"tangent",...(152<=_three.REVISION.replace(/\D+/g,"")?{TEXCOORD_0:"uv",TEXCOORD_1:"uv1",TEXCOORD_2:"uv2",TEXCOORD_3:"uv3"}:{TEXCOORD_0:"uv",TEXCOORD_1:"uv2"}),COLOR_0:"color",WEIGHTS_0:"skinWeight",JOINTS_0:"skinIndex"},PATH_PROPERTIES=
{scale:"scale",translation:"position",rotation:"quaternion",weights:"morphTargetInfluences"},INTERPOLATION={CUBICSPLINE:void 0,LINEAR:_three.InterpolateLinear,STEP:_three.InterpolateDiscrete},_identityMatrix=new _three.Matrix4;class GLTFParser{constructor(json={},options={}){this.json=json;this.extensions={};this.plugins={};this.options=options;this.cache=new GLTFRegistry;this.associations=new Map;this.primitiveCache={};this.nodeCache={};this.meshCache={refs:{},uses:{}};this.cameraCache={refs:{},
uses:{}};this.lightCache={refs:{},uses:{}};this.sourceCache={};this.textureCache={};this.nodeNamesUsed={};options=json=!1;let firefoxVersion=-1;"undefined"!==typeof navigator&&(json=!0===/^((?!chrome|android).)*safari/i.test(navigator.userAgent),firefoxVersion=(options=-1<navigator.userAgent.indexOf("Firefox"))?navigator.userAgent.match(/Firefox\/([0-9]+)\./)[1]:-1);this.textureLoader="undefined"===typeof createImageBitmap||json||options&&98>firefoxVersion?new _three.TextureLoader(this.options.manager):
new _three.ImageBitmapLoader(this.options.manager);this.textureLoader.setCrossOrigin(this.options.crossOrigin);this.textureLoader.setRequestHeader(this.options.requestHeader);this.fileLoader=new _three.FileLoader(this.options.manager);this.fileLoader.setResponseType("arraybuffer");"use-credentials"===this.options.crossOrigin&&this.fileLoader.setWithCredentials(!0)}setExtensions(extensions){this.extensions=extensions}setPlugins(plugins){this.plugins=plugins}parse(onLoad,onError){const parser=this,
json=this.json,extensions=this.extensions;this.cache.removeAll();this.nodeCache={};this._invokeAll(function(ext){return ext._markDefs&&ext._markDefs()});Promise.all(this._invokeAll(function(ext){return ext.beforeRoot&&ext.beforeRoot()})).then(function(){return Promise.all([parser.getDependencies("scene"),parser.getDependencies("animation"),parser.getDependencies("camera")])}).then(function(dependencies){const result={scene:dependencies[0][json.scene||0],scenes:dependencies[0],animations:dependencies[1],
cameras:dependencies[2],asset:json.asset,parser,userData:{}};addUnknownExtensionsToUserData(extensions,result,json);assignExtrasToUserData(result,json);Promise.all(parser._invokeAll(function(ext){return ext.afterRoot&&ext.afterRoot(result)})).then(function(){onLoad(result)})}).catch(onError)}_markDefs(){const nodeDefs=this.json.nodes||[];var skinDefs=this.json.skins||[];const meshDefs=this.json.meshes||[];for(let skinIndex=0,skinLength=skinDefs.length;skinIndex<skinLength;skinIndex++){const joints=
skinDefs[skinIndex].joints;for(let i=0,il=joints.length;i<il;i++)nodeDefs[joints[i]].isBone=!0}for(let nodeIndex=0,nodeLength=nodeDefs.length;nodeIndex<nodeLength;nodeIndex++)skinDefs=nodeDefs[nodeIndex],void 0!==skinDefs.mesh&&(this._addNodeRef(this.meshCache,skinDefs.mesh),void 0!==skinDefs.skin&&(meshDefs[skinDefs.mesh].isSkinnedMesh=!0)),void 0!==skinDefs.camera&&this._addNodeRef(this.cameraCache,skinDefs.camera)}_addNodeRef(cache,index){void 0!==index&&(void 0===cache.refs[index]&&(cache.refs[index]=
cache.uses[index]=0),cache.refs[index]++)}_getNodeRef(cache,index,object){if(1>=cache.refs[index])return object;const ref=object.clone(),updateMappings=(original,clone)=>{const mappings=this.associations.get(original);null!=mappings&&this.associations.set(clone,mappings);for(const [i,child]of original.children.entries())updateMappings(child,clone.children[i])};updateMappings(object,ref);ref.name+="_instance_"+cache.uses[index]++;return ref}_invokeOne(func){const extensions=Object.values(this.plugins);
extensions.push(this);for(let i=0;i<extensions.length;i++){const result=func(extensions[i]);if(result)return result}return null}_invokeAll(func){const extensions=Object.values(this.plugins);extensions.unshift(this);const pending=[];for(let i=0;i<extensions.length;i++){const result=func(extensions[i]);result&&pending.push(result)}return pending}getDependency(type,index){const cacheKey=type+":"+index;let dependency=this.cache.get(cacheKey);if(!dependency){switch(type){case "scene":dependency=this.loadScene(index);
break;case "node":dependency=this._invokeOne(function(ext){return ext.loadNode&&ext.loadNode(index)});break;case "mesh":dependency=this._invokeOne(function(ext){return ext.loadMesh&&ext.loadMesh(index)});break;case "accessor":dependency=this.loadAccessor(index);break;case "bufferView":dependency=this._invokeOne(function(ext){return ext.loadBufferView&&ext.loadBufferView(index)});break;case "buffer":dependency=this.loadBuffer(index);break;case "material":dependency=this._invokeOne(function(ext){return ext.loadMaterial&&
ext.loadMaterial(index)});break;case "texture":dependency=this._invokeOne(function(ext){return ext.loadTexture&&ext.loadTexture(index)});break;case "skin":dependency=this.loadSkin(index);break;case "animation":dependency=this._invokeOne(function(ext){return ext.loadAnimation&&ext.loadAnimation(index)});break;case "camera":dependency=this.loadCamera(index);break;default:if(dependency=this._invokeOne(function(ext){return ext!=this&&ext.getDependency&&ext.getDependency(type,index)}),!dependency)throw Error("Unknown type: "+
type);}this.cache.add(cacheKey,dependency)}return dependency}getDependencies(type){let dependencies=this.cache.get(type);if(!dependencies){const parser=this;dependencies=Promise.all((this.json[type+("mesh"===type?"es":"s")]||[]).map(function(def,index){return parser.getDependency(type,index)}));this.cache.add(type,dependencies)}return dependencies}loadBuffer(bufferIndex){const bufferDef=this.json.buffers[bufferIndex],loader=this.fileLoader;if(bufferDef.type&&"arraybuffer"!==bufferDef.type)throw Error("THREE.GLTFLoader: "+
bufferDef.type+" buffer type is not supported.");if(void 0===bufferDef.uri&&0===bufferIndex)return Promise.resolve(this.extensions[EXTENSIONS.KHR_BINARY_GLTF].body);const options=this.options;return new Promise(function(resolve,reject){loader.load(_three.LoaderUtils.resolveURL(bufferDef.uri,options.path),resolve,void 0,function(){reject(Error('THREE.GLTFLoader: Failed to load buffer "'+bufferDef.uri+'".'))})})}loadBufferView(bufferViewIndex){const bufferViewDef=this.json.bufferViews[bufferViewIndex];
return this.getDependency("buffer",bufferViewDef.buffer).then(function(buffer){const byteOffset=bufferViewDef.byteOffset||0;return buffer.slice(byteOffset,byteOffset+(bufferViewDef.byteLength||0))})}loadAccessor(accessorIndex){const parser=this,json=this.json,accessorDef=this.json.accessors[accessorIndex];if(void 0===accessorDef.bufferView&&void 0===accessorDef.sparse){accessorIndex=WEBGL_TYPE_SIZES[accessorDef.type];const normalized=!0===accessorDef.normalized,array=new WEBGL_COMPONENT_TYPES[accessorDef.componentType](accessorDef.count*
accessorIndex);return Promise.resolve(new _three.BufferAttribute(array,accessorIndex,normalized))}accessorIndex=[];void 0!==accessorDef.bufferView?accessorIndex.push(this.getDependency("bufferView",accessorDef.bufferView)):accessorIndex.push(null);void 0!==accessorDef.sparse&&(accessorIndex.push(this.getDependency("bufferView",accessorDef.sparse.indices.bufferView)),accessorIndex.push(this.getDependency("bufferView",accessorDef.sparse.values.bufferView)));return Promise.all(accessorIndex).then(function(bufferViews){var bufferView=
bufferViews[0];const itemSize=WEBGL_TYPE_SIZES[accessorDef.type],TypedArray=WEBGL_COMPONENT_TYPES[accessorDef.componentType];var elementBytes=TypedArray.BYTES_PER_ELEMENT,byteOffset=accessorDef.byteOffset||0,byteStride=void 0!==accessorDef.bufferView?json.bufferViews[accessorDef.bufferView].byteStride:void 0;const normalized=!0===accessorDef.normalized;if(byteStride&&byteStride!==elementBytes*itemSize){var array=Math.floor(byteOffset/byteStride);const ibCacheKey="InterleavedBuffer:"+accessorDef.bufferView+
":"+accessorDef.componentType+":"+array+":"+accessorDef.count;let ib=parser.cache.get(ibCacheKey);ib||(array=new TypedArray(bufferView,array*byteStride,accessorDef.count*byteStride/elementBytes),ib=new _three.InterleavedBuffer(array,byteStride/elementBytes),parser.cache.add(ibCacheKey,ib));elementBytes=new _three.InterleavedBufferAttribute(ib,itemSize,byteOffset%byteStride/elementBytes,normalized)}else array=null===bufferView?new TypedArray(accessorDef.count*itemSize):new TypedArray(bufferView,byteOffset,
accessorDef.count*itemSize),elementBytes=new _three.BufferAttribute(array,itemSize,normalized);if(void 0!==accessorDef.sparse){byteStride=accessorDef.sparse.values.byteOffset||0;byteOffset=new WEBGL_COMPONENT_TYPES[accessorDef.sparse.indices.componentType](bufferViews[1],accessorDef.sparse.indices.byteOffset||0,accessorDef.sparse.count*WEBGL_TYPE_SIZES.SCALAR);bufferViews=new TypedArray(bufferViews[2],byteStride,accessorDef.sparse.count*itemSize);null!==bufferView&&(elementBytes=new _three.BufferAttribute(elementBytes.array.slice(),
elementBytes.itemSize,elementBytes.normalized));for(let i=0,il=byteOffset.length;i<il;i++)if(bufferView=byteOffset[i],elementBytes.setX(bufferView,bufferViews[i*itemSize]),2<=itemSize&&elementBytes.setY(bufferView,bufferViews[i*itemSize+1]),3<=itemSize&&elementBytes.setZ(bufferView,bufferViews[i*itemSize+2]),4<=itemSize&&elementBytes.setW(bufferView,bufferViews[i*itemSize+3]),5<=itemSize)throw Error("THREE.GLTFLoader: Unsupported itemSize in sparse BufferAttribute.");}return elementBytes})}loadTexture(textureIndex){var json=
this.json,options=this.options;const sourceIndex=json.textures[textureIndex].source,sourceDef=json.images[sourceIndex];json=this.textureLoader;sourceDef.uri&&(options=options.manager.getHandler(sourceDef.uri),null!==options&&(json=options));return this.loadTextureImage(textureIndex,sourceIndex,json)}loadTextureImage(textureIndex,sourceIndex,loader){const parser=this,json=this.json,textureDef=json.textures[textureIndex],sourceDef=json.images[sourceIndex],cacheKey=(sourceDef.uri||sourceDef.bufferView)+
":"+textureDef.sampler;if(this.textureCache[cacheKey])return this.textureCache[cacheKey];sourceIndex=this.loadImageSource(sourceIndex,loader).then(function(texture){texture.flipY=!1;texture.name=textureDef.name||sourceDef.name||"";""===texture.name&&"string"===typeof sourceDef.uri&&!1===sourceDef.uri.startsWith("data:image/")&&(texture.name=sourceDef.uri);const sampler=(json.samplers||{})[textureDef.sampler]||{};texture.magFilter=WEBGL_FILTERS[sampler.magFilter]||_three.LinearFilter;texture.minFilter=
WEBGL_FILTERS[sampler.minFilter]||_three.LinearMipmapLinearFilter;texture.wrapS=WEBGL_WRAPPINGS[sampler.wrapS]||_three.RepeatWrapping;texture.wrapT=WEBGL_WRAPPINGS[sampler.wrapT]||_three.RepeatWrapping;parser.associations.set(texture,{textures:textureIndex});return texture}).catch(function(){return null});return this.textureCache[cacheKey]=sourceIndex}loadImageSource(sourceIndex,loader){var json=this.json;const options=this.options;if(void 0!==this.sourceCache[sourceIndex])return this.sourceCache[sourceIndex].then(texture=>
texture.clone());const sourceDef=json.images[sourceIndex],URL=self.URL||self.webkitURL;let sourceURI=sourceDef.uri||"",isObjectURL=!1;if(void 0!==sourceDef.bufferView)sourceURI=this.getDependency("bufferView",sourceDef.bufferView).then(function(bufferView){isObjectURL=!0;bufferView=new Blob([bufferView],{type:sourceDef.mimeType});return sourceURI=URL.createObjectURL(bufferView)});else if(void 0===sourceDef.uri)throw Error("THREE.GLTFLoader: Image "+sourceIndex+" is missing URI and bufferView");json=
Promise.resolve(sourceURI).then(function(sourceURI){return new Promise(function(resolve,reject){let onLoad=resolve;!0===loader.isImageBitmapLoader&&(onLoad=function(imageBitmap){imageBitmap=new _three.Texture(imageBitmap);imageBitmap.needsUpdate=!0;resolve(imageBitmap)});loader.load(_three.LoaderUtils.resolveURL(sourceURI,options.path),onLoad,void 0,reject)})}).then(function(texture){!0===isObjectURL&&URL.revokeObjectURL(sourceURI);var JSCompiler_temp_const=texture.userData,JSCompiler_temp;(JSCompiler_temp=
sourceDef.mimeType)||(JSCompiler_temp=sourceDef.uri,JSCompiler_temp=0<JSCompiler_temp.search(/\.jpe?g($|\?)/i)||0===JSCompiler_temp.search(/^data:image\/jpeg/)?"image/jpeg":0<JSCompiler_temp.search(/\.webp($|\?)/i)||0===JSCompiler_temp.search(/^data:image\/webp/)?"image/webp":"image/png");JSCompiler_temp_const.mimeType=JSCompiler_temp;return texture}).catch(function(error){console.error("THREE.GLTFLoader: Couldn't load texture",sourceURI);throw error;});return this.sourceCache[sourceIndex]=json}assignTexture(materialParams,
mapName,mapDef,encoding){const parser=this;return this.getDependency("texture",mapDef.index).then(function(texture){if(!texture)return null;void 0!==mapDef.texCoord&&0<mapDef.texCoord&&(texture=texture.clone(),texture.channel=mapDef.texCoord);if(parser.extensions[EXTENSIONS.KHR_TEXTURE_TRANSFORM]){const transform=void 0!==mapDef.extensions?mapDef.extensions[EXTENSIONS.KHR_TEXTURE_TRANSFORM]:void 0;if(transform){const gltfReference=parser.associations.get(texture);texture=parser.extensions[EXTENSIONS.KHR_TEXTURE_TRANSFORM].extendTexture(texture,
transform);parser.associations.set(texture,gltfReference)}}void 0!==encoding&&(texture.encoding=encoding);return materialParams[mapName]=texture})}assignFinalMaterial(mesh){var geometry=mesh.geometry;let material=mesh.material;const useDerivativeTangents=void 0===geometry.attributes.tangent,useVertexColors=void 0!==geometry.attributes.color;geometry=void 0===geometry.attributes.normal;if(mesh.isPoints){var cacheKey="PointsMaterial:"+material.uuid,pointsMaterial=this.cache.get(cacheKey);pointsMaterial||
...
pointsMaterial.color.copy(material.color),pointsMaterial.map=material.map,this.cache.add(cacheKey,pointsMaterial)),material=pointsMaterial);if(useDerivativeTangents||useVertexColors||geometry)cacheKey="ClonedMaterial:"+material.uuid+":",useDerivativeTangents&&(cacheKey+="derivative-tangents:"),useVertexColors&&(cacheKey+="vertex-colors:"),geometry&&(cacheKey+="flat-shading:"),pointsMaterial=this.cache.get(cacheKey),pointsMaterial||(pointsMaterial=material.clone(),useVertexColors&&(pointsMaterial.vertexColors=
!0),geometry&&(pointsMaterial.flatShading=!0),useDerivativeTangents&&(pointsMaterial.normalScale&&(pointsMaterial.normalScale.y*=-1),pointsMaterial.clearcoatNormalScale&&(pointsMaterial.clearcoatNormalScale.y*=-1)),this.cache.add(cacheKey,pointsMaterial),this.associations.set(pointsMaterial,this.associations.get(material))),material=pointsMaterial;mesh.material=material}getMaterialType(){return _three.MeshStandardMaterial}loadMaterial(materialIndex){const parser=this,extensions=this.extensions,materialDef=
this.json.materials[materialIndex];let materialType;const materialParams={},pending=[];if((materialDef.extensions||{})[EXTENSIONS.KHR_MATERIALS_UNLIT]){var kmuExtension=extensions[EXTENSIONS.KHR_MATERIALS_UNLIT];materialType=kmuExtension.getMaterialType();pending.push(kmuExtension.extendParams(materialParams,materialDef,parser))}else{kmuExtension=materialDef.pbrMetallicRoughness||{};materialParams.color=new _three.Color(1,1,1);materialParams.opacity=1;if(Array.isArray(kmuExtension.baseColorFactor)){const array=
kmuExtension.baseColorFactor;materialParams.color.fromArray(array);materialParams.opacity=array[3]}void 0!==kmuExtension.baseColorTexture&&pending.push(parser.assignTexture(materialParams,"map",kmuExtension.baseColorTexture,_three.sRGBEncoding));materialParams.metalness=void 0!==kmuExtension.metallicFactor?kmuExtension.metallicFactor:1;materialParams.roughness=void 0!==kmuExtension.roughnessFactor?kmuExtension.roughnessFactor:1;void 0!==kmuExtension.metallicRoughnessTexture&&(pending.push(parser.assignTexture(materialParams,
"metalnessMap",kmuExtension.metallicRoughnessTexture)),pending.push(parser.assignTexture(materialParams,"roughnessMap",kmuExtension.metallicRoughnessTexture)));materialType=this._invokeOne(function(ext){return ext.getMaterialType&&ext.getMaterialType(materialIndex)});pending.push(Promise.all(this._invokeAll(function(ext){return ext.extendMaterialParams&&ext.extendMaterialParams(materialIndex,materialParams)})))}!0===materialDef.doubleSided&&(materialParams.side=_three.DoubleSide);kmuExtension=materialDef.alphaMode||
"OPAQUE";"BLEND"===kmuExtension?(materialParams.transparent=!0,materialParams.depthWrite=!1):(materialParams.transparent=!1,"MASK"===kmuExtension&&(materialParams.alphaTest=void 0!==materialDef.alphaCutoff?materialDef.alphaCutoff:.5));void 0!==materialDef.normalTexture&&materialType!==_three.MeshBasicMaterial&&(pending.push(parser.assignTexture(materialParams,"normalMap",materialDef.normalTexture)),materialParams.normalScale=new _three.Vector2(1,1),void 0!==materialDef.normalTexture.scale&&(kmuExtension=
materialDef.normalTexture.scale,materialParams.normalScale.set(kmuExtension,kmuExtension)));void 0!==materialDef.occlusionTexture&&materialType!==_three.MeshBasicMaterial&&(pending.push(parser.assignTexture(materialParams,"aoMap",materialDef.occlusionTexture)),void 0!==materialDef.occlusionTexture.strength&&(materialParams.aoMapIntensity=materialDef.occlusionTexture.strength));void 0!==materialDef.emissiveFactor&&materialType!==_three.MeshBasicMaterial&&(materialParams.emissive=(new _three.Color).fromArray(materialDef.emissiveFactor));
void 0!==materialDef.emissiveTexture&&materialType!==_three.MeshBasicMaterial&&pending.push(parser.assignTexture(materialParams,"emissiveMap",materialDef.emissiveTexture,_three.sRGBEncoding));return Promise.all(pending).then(function(){const material=new materialType(materialParams);materialDef.name&&(material.name=materialDef.name);assignExtrasToUserData(material,materialDef);parser.associations.set(material,{materials:materialIndex});materialDef.extensions&&addUnknownExtensionsToUserData(extensions,
material,materialDef);return material})}createUniqueName(originalName){let name=originalName=_three.PropertyBinding.sanitizeNodeName(originalName||"");for(let i=1;this.nodeNamesUsed[name];++i)name=originalName+"_"+i;this.nodeNamesUsed[name]=!0;return name}loadGeometries(primitives){function createDracoPrimitive(primitive){return extensions[EXTENSIONS.KHR_DRACO_MESH_COMPRESSION].decodePrimitive(primitive,parser).then(function(geometry){return addPrimitiveAttributes(geometry,primitive,parser)})}const parser=
this,extensions=this.extensions,cache=this.primitiveCache,pending=[];for(let i=0,il=primitives.length;i<il;i++){const primitive=primitives[i],cacheKey=createPrimitiveKey(primitive);var cached=cache[cacheKey];cached?pending.push(cached.promise):(cached=primitive.extensions&&primitive.extensions[EXTENSIONS.KHR_DRACO_MESH_COMPRESSION]?createDracoPrimitive(primitive):addPrimitiveAttributes(new _three.BufferGeometry,primitive,parser),cache[cacheKey]={primitive,promise:cached},pending.push(cached))}return Promise.all(pending)}loadMesh(meshIndex){const parser=
this,extensions=this.extensions,meshDef$jscomp$0=this.json.meshes[meshIndex],primitives=meshDef$jscomp$0.primitives,pending=[];for(let i=0,il=primitives.length;i<il;i++){const material=void 0===primitives[i].material?createDefaultMaterial(this.cache):this.getDependency("material",primitives[i].material);pending.push(material)}pending.push(parser.loadGeometries(primitives));return Promise.all(pending).then(function(results){var materials=results.slice(0,results.length-1);const geometries=results[results.length-
1];results=[];for(let i$jscomp$0=0,il$jscomp$0=geometries.length;i$jscomp$0<il$jscomp$0;i$jscomp$0++){var geometry=geometries[i$jscomp$0];const primitive=primitives[i$jscomp$0];var mesh=void 0;mesh=materials[i$jscomp$0];if(primitive.mode===WEBGL_CONSTANTS.TRIANGLES||primitive.mode===WEBGL_CONSTANTS.TRIANGLE_STRIP||primitive.mode===WEBGL_CONSTANTS.TRIANGLE_FAN||void 0===primitive.mode)mesh=!0===meshDef$jscomp$0.isSkinnedMesh?new _three.SkinnedMesh(geometry,mesh):new _three.Mesh(geometry,mesh),!0===
mesh.isSkinnedMesh&&mesh.normalizeSkinWeights(),primitive.mode===WEBGL_CONSTANTS.TRIANGLE_STRIP?mesh.geometry=(0,_BufferGeometryUtils.toTrianglesDrawMode)(mesh.geometry,_three.TriangleStripDrawMode):primitive.mode===WEBGL_CONSTANTS.TRIANGLE_FAN&&(mesh.geometry=(0,_BufferGeometryUtils.toTrianglesDrawMode)(mesh.geometry,_three.TriangleFanDrawMode));else if(primitive.mode===WEBGL_CONSTANTS.LINES)mesh=new _three.LineSegments(geometry,mesh);else if(primitive.mode===WEBGL_CONSTANTS.LINE_STRIP)mesh=new _three.Line(geometry,
mesh);else if(primitive.mode===WEBGL_CONSTANTS.LINE_LOOP)mesh=new _three.LineLoop(geometry,mesh);else if(primitive.mode===WEBGL_CONSTANTS.POINTS)mesh=new _three.Points(geometry,mesh);else throw Error("THREE.GLTFLoader: Primitive mode unsupported: "+primitive.mode);if(0<Object.keys(mesh.geometry.morphAttributes).length){geometry=mesh;var meshDef=meshDef$jscomp$0;geometry.updateMorphTargets();if(void 0!==meshDef.weights)for(let i=0,il=meshDef.weights.length;i<il;i++)geometry.morphTargetInfluences[i]=
meshDef.weights[i];if(meshDef.extras&&Array.isArray(meshDef.extras.targetNames))if(meshDef=meshDef.extras.targetNames,geometry.morphTargetInfluences.length===meshDef.length){geometry.morphTargetDictionary={};for(let i=0,il=meshDef.length;i<il;i++)geometry.morphTargetDictionary[meshDef[i]]=i}else console.warn("THREE.GLTFLoader: Invalid extras.targetNames length. Ignoring names.")}mesh.name=parser.createUniqueName(meshDef$jscomp$0.name||"mesh_"+meshIndex);assignExtrasToUserData(mesh,meshDef$jscomp$0);
primitive.extensions&&addUnknownExtensionsToUserData(extensions,mesh,primitive);parser.assignFinalMaterial(mesh);results.push(mesh)}for(let i=0,il=results.length;i<il;i++)parser.associations.set(results[i],{meshes:meshIndex,primitives:i});if(1===results.length)return results[0];materials=new _three.Group;parser.associations.set(materials,{meshes:meshIndex});for(let i=0,il=results.length;i<il;i++)materials.add(results[i]);return materials})}loadCamera(cameraIndex){let camera;cameraIndex=this.json.cameras[cameraIndex];
const params=cameraIndex[cameraIndex.type];if(params)return"perspective"===cameraIndex.type?camera=new _three.PerspectiveCamera(_three.MathUtils.radToDeg(params.yfov),params.aspectRatio||1,params.znear||1,params.zfar||2E6):"orthographic"===cameraIndex.type&&(camera=new _three.OrthographicCamera(-params.xmag,params.xmag,params.ymag,-params.ymag,params.znear,params.zfar)),cameraIndex.name&&(camera.name=this.createUniqueName(cameraIndex.name)),assignExtrasToUserData(camera,cameraIndex),Promise.resolve(camera);
console.warn("THREE.GLTFLoader: Missing camera parameters.")}loadSkin(skinIndex){const skinDef=this.json.skins[skinIndex];skinIndex=[];for(let i=0,il=skinDef.joints.length;i<il;i++)skinIndex.push(this._loadNodeShallow(skinDef.joints[i]));void 0!==skinDef.inverseBindMatrices?skinIndex.push(this.getDependency("accessor",skinDef.inverseBindMatrices)):skinIndex.push(null);return Promise.all(skinIndex).then(function(results){const inverseBindMatrices=results.pop(),bones=[],boneInverses=[];for(let i=0,
il=results.length;i<il;i++){var jointNode=results[i];jointNode?(bones.push(jointNode),jointNode=new _three.Matrix4,null!==inverseBindMatrices&&jointNode.fromArray(inverseBindMatrices.array,16*i),boneInverses.push(jointNode)):console.warn('THREE.GLTFLoader: Joint "%s" could not be found.',skinDef.joints[i])}return new _three.Skeleton(bones,boneInverses)})}loadAnimation(animationIndex){const animationDef=this.json.animations[animationIndex],animationName=animationDef.name?animationDef.name:"animation_"+
animationIndex;animationIndex=[];const pendingInputAccessors=[],pendingOutputAccessors=[],pendingSamplers=[],pendingTargets=[];for(let i=0,il=animationDef.channels.length;i<il;i++){var channel=animationDef.channels[i];const sampler=animationDef.samplers[channel.sampler];channel=channel.target;const input=void 0!==animationDef.parameters?animationDef.parameters[sampler.input]:sampler.input,output=void 0!==animationDef.parameters?animationDef.parameters[sampler.output]:sampler.output;void 0!==channel.node&&
(animationIndex.push(this.getDependency("node",channel.node)),pendingInputAccessors.push(this.getDependency("accessor",input)),pendingOutputAccessors.push(this.getDependency("accessor",output)),pendingSamplers.push(sampler),pendingTargets.push(channel))}return Promise.all([Promise.all(animationIndex),Promise.all(pendingInputAccessors),Promise.all(pendingOutputAccessors),Promise.all(pendingSamplers),Promise.all(pendingTargets)]).then(function(dependencies){const nodes=dependencies[0],inputAccessors=
dependencies[1],outputAccessors=dependencies[2],samplers=dependencies[3];dependencies=dependencies[4];const tracks=[];for(let i=0,il=nodes.length;i<il;i++){var node=nodes[i];const inputAccessor=inputAccessors[i];var outputAccessor=outputAccessors[i];const sampler=samplers[i],target=dependencies[i];if(void 0===node)continue;node.updateMatrix();let TypedKeyframeTrack;switch(PATH_PROPERTIES[target.path]){case PATH_PROPERTIES.weights:TypedKeyframeTrack=_three.NumberKeyframeTrack;break;case PATH_PROPERTIES.rotation:TypedKeyframeTrack=
_three.QuaternionKeyframeTrack;break;default:TypedKeyframeTrack=_three.VectorKeyframeTrack}var targetName=node.name?node.name:node.uuid;const interpolation=void 0!==sampler.interpolation?INTERPOLATION[sampler.interpolation]:_three.InterpolateLinear,targetNames=[];PATH_PROPERTIES[target.path]===PATH_PROPERTIES.weights?node.traverse(function(object){object.morphTargetInfluences&&targetNames.push(object.name?object.name:object.uuid)}):targetNames.push(targetName);node=outputAccessor.array;if(outputAccessor.normalized){outputAccessor=
getNormalizedComponentScale(node.constructor);targetName=new Float32Array(node.length);for(let j=0,jl=node.length;j<jl;j++)targetName[j]=node[j]*outputAccessor;node=targetName}for(let j=0,jl=targetNames.length;j<jl;j++)outputAccessor=new TypedKeyframeTrack(targetNames[j]+"."+PATH_PROPERTIES[target.path],inputAccessor.array,node,interpolation),"CUBICSPLINE"===sampler.interpolation&&(outputAccessor.createInterpolant=function(result){return new (this instanceof _three.QuaternionKeyframeTrack?GLTFCubicSplineQuaternionInterpolant:
GLTFCubicSplineInterpolant)(this.times,this.values,this.getValueSize()/3,result)},outputAccessor.createInterpolant.isInterpolantFactoryMethodGLTFCubicSpline=!0),tracks.push(outputAccessor)}return new _three.AnimationClip(animationName,void 0,tracks)})}createNodeMesh(nodeIndex){const parser=this,nodeDef=this.json.nodes[nodeIndex];return void 0===nodeDef.mesh?null:parser.getDependency("mesh",nodeDef.mesh).then(function(mesh){mesh=parser._getNodeRef(parser.meshCache,nodeDef.mesh,mesh);void 0!==nodeDef.weights&&
mesh.traverse(function(o){if(o.isMesh)for(let i=0,il=nodeDef.weights.length;i<il;i++)o.morphTargetInfluences[i]=nodeDef.weights[i]});return mesh})}loadNode(nodeIndex){var nodeDef=this.json.nodes[nodeIndex];nodeIndex=this._loadNodeShallow(nodeIndex);const childPending=[],childrenDef=nodeDef.children||[];for(let i=0,il=childrenDef.length;i<il;i++)childPending.push(this.getDependency("node",childrenDef[i]));nodeDef=void 0===nodeDef.skin?Promise.resolve(null):this.getDependency("skin",nodeDef.skin);return Promise.all([nodeIndex,
Promise.all(childPending),nodeDef]).then(function(results){const node=results[0],children=results[1],skeleton=results[2];null!==skeleton&&node.traverse(function(mesh){mesh.isSkinnedMesh&&mesh.bind(skeleton,_identityMatrix)});for(let i=0,il=children.length;i<il;i++)node.add(children[i]);return node})}_loadNodeShallow(nodeIndex){const extensions=this.extensions,parser=this;if(void 0!==this.nodeCache[nodeIndex])return this.nodeCache[nodeIndex];const nodeDef=this.json.nodes[nodeIndex],nodeName=nodeDef.name?
parser.createUniqueName(nodeDef.name):"",pending=[],meshPromise=parser._invokeOne(function(ext){return ext.createNodeMesh&&ext.createNodeMesh(nodeIndex)});meshPromise&&pending.push(meshPromise);void 0!==nodeDef.camera&&pending.push(parser.getDependency("camera",nodeDef.camera).then(function(camera){return parser._getNodeRef(parser.cameraCache,nodeDef.camera,camera)}));parser._invokeAll(function(ext){return ext.createNodeAttachment&&ext.createNodeAttachment(nodeIndex)}).forEach(function(promise){pending.push(promise)});
this.nodeCache[nodeIndex]=Promise.all(pending).then(function(objects){let node;node=!0===nodeDef.isBone?new _three.Bone:1<objects.length?new _three.Group:1===objects.length?objects[0]:new _three.Object3D;if(node!==objects[0])for(let i=0,il=objects.length;i<il;i++)node.add(objects[i]);nodeDef.name&&(node.userData.name=nodeDef.name,node.name=nodeName);assignExtrasToUserData(node,nodeDef);nodeDef.extensions&&addUnknownExtensionsToUserData(extensions,node,nodeDef);void 0!==nodeDef.matrix?(objects=new _three.Matrix4,
objects.fromArray(nodeDef.matrix),node.applyMatrix4(objects)):(void 0!==nodeDef.translation&&node.position.fromArray(nodeDef.translation),void 0!==nodeDef.rotation&&node.quaternion.fromArray(nodeDef.rotation),void 0!==nodeDef.scale&&node.scale.fromArray(nodeDef.scale));parser.associations.has(node)||parser.associations.set(node,{});parser.associations.get(node).nodes=nodeIndex;return node});return this.nodeCache[nodeIndex]}loadScene(sceneIndex){var extensions=this.extensions;sceneIndex=this.json.scenes[sceneIndex];
const parser=this,scene=new _three.Group;sceneIndex.name&&(scene.name=parser.createUniqueName(sceneIndex.name));assignExtrasToUserData(scene,sceneIndex);sceneIndex.extensions&&addUnknownExtensionsToUserData(extensions,scene,sceneIndex);extensions=sceneIndex.nodes||[];sceneIndex=[];for(let i=0,il=extensions.length;i<il;i++)sceneIndex.push(parser.getDependency("node",extensions[i]));return Promise.all(sceneIndex).then(function(nodes){for(let i=0,il=nodes.length;i<il;i++)scene.add(nodes[i]);parser.associations=
(node$jscomp$0=>{const reducedAssociations=new Map;for(const [key,value]of parser.associations)(key instanceof _three.Material||key instanceof _three.Texture)&&reducedAssociations.set(key,value);node$jscomp$0.traverse(node=>{const mappings=parser.associations.get(node);null!=mappings&&reducedAssociations.set(node,mappings)});return reducedAssociations})(scene);return scene})}}}
//# sourceMappingURL=module$node_modules$three_stdlib$loaders$GLTFLoader.js.map

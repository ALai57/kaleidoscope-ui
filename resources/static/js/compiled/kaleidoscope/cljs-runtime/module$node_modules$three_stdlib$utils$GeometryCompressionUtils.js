...
PackedPhongMaterial||(mesh.material=(new PackedPhongMaterial).copy(mesh.material));mesh.material.defines.USE_PACKED_POSITION=0;mesh.material.uniforms.quantizeMatPos.value=result;mesh.material.uniforms.quantizeMatPos.needsUpdate=!0}},compressUvs:function(mesh){mesh.geometry||console.error("Mesh must contain geometry property. ");var uvs=mesh.geometry.attributes.uv;uvs||console.error("Geometry must contain uv attribute. ");if(!uvs.isPacked){var JSCompiler_object_inline_min_9178=Infinity,JSCompiler_object_inline_max_9179=
-Infinity;uvs=uvs.array;for(var i=0;i<uvs.length;i++)JSCompiler_object_inline_min_9178=Math.min(JSCompiler_object_inline_min_9178,uvs[i]),JSCompiler_object_inline_max_9179=Math.max(JSCompiler_object_inline_max_9179,uvs[i]);if(-1<=JSCompiler_object_inline_min_9178&&1>=JSCompiler_object_inline_max_9179){JSCompiler_object_inline_min_9178=new Uint16Array(uvs.length);for(JSCompiler_object_inline_max_9179=0;JSCompiler_object_inline_max_9179<uvs.length;JSCompiler_object_inline_max_9179+=2)i=this.EncodingFuncs.defaultEncode(uvs[JSCompiler_object_inline_max_9179],
uvs[JSCompiler_object_inline_max_9179+1],0,2),JSCompiler_object_inline_min_9178[JSCompiler_object_inline_max_9179]=i[0],JSCompiler_object_inline_min_9178[JSCompiler_object_inline_max_9179+1]=i[1];mesh.geometry.setAttribute("uv",new _three.BufferAttribute(JSCompiler_object_inline_min_9178,2,!0));mesh.geometry.attributes.uv.isPacked=!0;mesh.geometry.attributes.uv.needsUpdate=!0;mesh.geometry.attributes.uv.bytes=2*JSCompiler_object_inline_min_9178.length;mesh.material instanceof PackedPhongMaterial||
(mesh.material=(new PackedPhongMaterial).copy(mesh.material));mesh.material.defines.USE_PACKED_UV=0}else JSCompiler_object_inline_min_9178=this.EncodingFuncs.quantizedEncodeUV(uvs,2),mesh.geometry.setAttribute("uv",new _three.BufferAttribute(JSCompiler_object_inline_min_9178.quantized,2)),mesh.geometry.attributes.uv.isPacked=!0,mesh.geometry.attributes.uv.needsUpdate=!0,mesh.geometry.attributes.uv.bytes=2*JSCompiler_object_inline_min_9178.quantized.length,mesh.material instanceof PackedPhongMaterial||
(mesh.material=(new PackedPhongMaterial).copy(mesh.material)),mesh.material.defines.USE_PACKED_UV=1,mesh.material.uniforms.quantizeMatUV.value=JSCompiler_object_inline_min_9178.decodeMat,mesh.material.uniforms.quantizeMatUV.needsUpdate=!0}},EncodingFuncs:{defaultEncode:function(x,y,z,bytes){if(1==bytes)return new Uint8Array([Math.round(127.5*(x+1)),Math.round(127.5*(y+1)),Math.round(127.5*(z+1))]);if(2==bytes)return new Uint16Array([Math.round(32767.5*(x+1)),Math.round(32767.5*(y+1)),Math.round(32767.5*
...
if(1==bytes){quantized=new Uint8Array(array.length);var segments=255}else 2==bytes?(quantized=new Uint16Array(array.length),segments=65535):console.error("number of bytes error! ");bytes=new _three.Matrix4;const min=new Float32Array(3);var max=new Float32Array(3);min[0]=min[1]=min[2]=Number.MAX_VALUE;max[0]=max[1]=max[2]=-Number.MAX_VALUE;for(let i=0;i<array.length;i+=3)min[0]=Math.min(min[0],array[i+0]),min[1]=Math.min(min[1],array[i+1]),min[2]=Math.min(min[2],array[i+2]),max[0]=Math.max(max[0],
array[i+0]),max[1]=Math.max(max[1],array[i+1]),max[2]=Math.max(max[2],array[i+2]);bytes.scale(new _three.Vector3((max[0]-min[0])/segments,(max[1]-min[1])/segments,(max[2]-min[2])/segments));bytes.elements[12]=min[0];bytes.elements[13]=min[1];bytes.elements[14]=min[2];bytes.transpose();segments=new Float32Array([max[0]!==min[0]?segments/(max[0]-min[0]):0,max[1]!==min[1]?segments/(max[1]-min[1]):0,max[2]!==min[2]?segments/(max[2]-min[2]):0]);for(max=0;max<array.length;max+=3)quantized[max+0]=Math.floor((array[max+
0]-min[0])*segments[0]),quantized[max+1]=Math.floor((array[max+1]-min[1])*segments[1]),quantized[max+2]=Math.floor((array[max+2]-min[2])*segments[2]);return{quantized,decodeMat:bytes}},quantizedEncodeUV:function(array,bytes){let quantized;if(1==bytes){quantized=new Uint8Array(array.length);var segments=255}else 2==bytes?(quantized=new Uint16Array(array.length),segments=65535):console.error("number of bytes error! ");bytes=new _three.Matrix3;const min=new Float32Array(2);var max=new Float32Array(2);
min[0]=min[1]=Number.MAX_VALUE;max[0]=max[1]=-Number.MAX_VALUE;for(let i=0;i<array.length;i+=2)min[0]=Math.min(min[0],array[i+0]),min[1]=Math.min(min[1],array[i+1]),max[0]=Math.max(max[0],array[i+0]),max[1]=Math.max(max[1],array[i+1]);bytes.scale((max[0]-min[0])/segments,(max[1]-min[1])/segments);bytes.elements[6]=min[0];bytes.elements[7]=min[1];bytes.transpose();segments=new Float32Array([max[0]!==min[0]?segments/(max[0]-min[0]):0,max[1]!==min[1]?segments/(max[1]-min[1]):0]);for(max=0;max<array.length;max+=
2)quantized[max+0]=Math.floor((array[max+0]-min[0])*segments[0]),quantized[max+1]=Math.floor((array[max+1]-min[1])*segments[1]);return{quantized,decodeMat:bytes}}}};class PackedPhongMaterial extends _three.MeshPhongMaterial{constructor(parameters){super();this.defines={};this.type="PackedPhongMaterial";this.uniforms=_three.UniformsUtils.merge([_three.ShaderLib.phong.uniforms,{quantizeMatPos:{value:null},quantizeMatUV:{value:null}}]);this.vertexShader=["#define PHONG\nvarying vec3 vViewPosition;\n#ifndef FLAT_SHADED\nvarying vec3 vNormal;\n#endif",
_three.ShaderChunk.common,_three.ShaderChunk.uv_pars_vertex,_three.ShaderChunk.uv2_pars_vertex,_three.ShaderChunk.displacementmap_pars_vertex,_three.ShaderChunk.envmap_pars_vertex,_three.ShaderChunk.color_pars_vertex,_three.ShaderChunk.fog_pars_vertex,_three.ShaderChunk.morphtarget_pars_vertex,_three.ShaderChunk.skinning_pars_vertex,_three.ShaderChunk.shadowmap_pars_vertex,_three.ShaderChunk.logdepthbuf_pars_vertex,_three.ShaderChunk.clipping_planes_pars_vertex,"#ifdef USE_PACKED_NORMAL\n\t\t\t\t\t#if USE_PACKED_NORMAL \x3d\x3d 0\n\t\t\t\t\t\tvec3 decodeNormal(vec3 packedNormal)\n\t\t\t\t\t\t{\n\t\t\t\t\t\t\tfloat x \x3d packedNormal.x * 2.0 - 1.0;\n\t\t\t\t\t\t\tfloat y \x3d packedNormal.y * 2.0 - 1.0;\n\t\t\t\t\t\t\tvec2 scth \x3d vec2(sin(x * PI), cos(x * PI));\n\t\t\t\t\t\t\tvec2 scphi \x3d vec2(sqrt(1.0 - y * y), y);\n\t\t\t\t\t\t\treturn normalize( vec3(scth.y * scphi.x, scth.x * scphi.x, scphi.y) );\n\t\t\t\t\t\t}\n\t\t\t\t\t#endif\n\n\t\t\t\t\t#if USE_PACKED_NORMAL \x3d\x3d 1\n\t\t\t\t\t\tvec3 decodeNormal(vec3 packedNormal)\n\t\t\t\t\t\t{\n\t\t\t\t\t\t\tvec3 v \x3d vec3(packedNormal.xy, 1.0 - abs(packedNormal.x) - abs(packedNormal.y));\n\t\t\t\t\t\t\tif (v.z \x3c 0.0)\n\t\t\t\t\t\t\t{\n\t\t\t\t\t\t\t\tv.xy \x3d (1.0 - abs(v.yx)) * vec2((v.x \x3e\x3d 0.0) ? +1.0 : -1.0, (v.y \x3e\x3d 0.0) ? +1.0 : -1.0);\n\t\t\t\t\t\t\t}\n\t\t\t\t\t\t\treturn normalize(v);\n\t\t\t\t\t\t}\n\t\t\t\t\t#endif\n\n\t\t\t\t\t#if USE_PACKED_NORMAL \x3d\x3d 2\n\t\t\t\t\t\tvec3 decodeNormal(vec3 packedNormal)\n\t\t\t\t\t\t{\n\t\t\t\t\t\t\tvec3 v \x3d (packedNormal * 2.0) - 1.0;\n\t\t\t\t\t\t\treturn normalize(v);\n\t\t\t\t\t\t}\n\t\t\t\t\t#endif\n\t\t\t\t#endif\n#ifdef USE_PACKED_POSITION\n\t\t\t\t\t#if USE_PACKED_POSITION \x3d\x3d 0\n\t\t\t\t\t\tuniform mat4 quantizeMatPos;\n\t\t\t\t\t#endif\n\t\t\t\t#endif\n#ifdef USE_PACKED_UV\n\t\t\t\t\t#if USE_PACKED_UV \x3d\x3d 1\n\t\t\t\t\t\tuniform mat3 quantizeMatUV;\n\t\t\t\t\t#endif\n\t\t\t\t#endif\n#ifdef USE_PACKED_UV\n\t\t\t\t\t#if USE_PACKED_UV \x3d\x3d 0\n\t\t\t\t\t\tvec2 decodeUV(vec2 packedUV)\n\t\t\t\t\t\t{\n\t\t\t\t\t\t\tvec2 uv \x3d (packedUV * 2.0) - 1.0;\n\t\t\t\t\t\t\treturn uv;\n\t\t\t\t\t\t}\n\t\t\t\t\t#endif\n\n\t\t\t\t\t#if USE_PACKED_UV \x3d\x3d 1\n\t\t\t\t\t\tvec2 decodeUV(vec2 packedUV)\n\t\t\t\t\t\t{\n\t\t\t\t\t\t\tvec2 uv \x3d ( vec3(packedUV, 1.0) * quantizeMatUV ).xy;\n\t\t\t\t\t\t\treturn uv;\n\t\t\t\t\t\t}\n\t\t\t\t\t#endif\n\t\t\t\t#endif\nvoid main() {",
...
"}"].join("\n");this.setValues(parameters)}}exports.PackedPhongMaterial=PackedPhongMaterial}
//# sourceMappingURL=module$node_modules$three_stdlib$utils$GeometryCompressionUtils.js.map

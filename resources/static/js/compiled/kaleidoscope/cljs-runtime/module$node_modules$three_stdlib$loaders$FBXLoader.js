shadow$provide.module$node_modules$three_stdlib$loaders$FBXLoader=function(global,require,module,exports){function getFbxVersion(text){if(text=text.match(/FBXVersion: (\d+)/))return parseInt(text[1]);throw Error("THREE.FBXLoader: Cannot find the version number for the file given.");}function convertFBXTimeToSeconds(time){return time/46186158E3}function getData(polygonVertexIndex,polygonIndex,vertexIndex,infoObject){let index;switch(infoObject.mappingType){case "ByPolygonVertex":index=polygonVertexIndex;
break;case "ByPolygon":index=polygonIndex;break;case "ByVertice":index=vertexIndex;break;case "AllSame":index=infoObject.indices[0];break;default:console.warn("THREE.FBXLoader: unknown attribute mapping type "+infoObject.mappingType)}"IndexToDirect"===infoObject.referenceType&&(index=infoObject.indices[index]);polygonVertexIndex=index*infoObject.dataSize;polygonIndex=dataArray;vertexIndex=infoObject.buffer;infoObject=polygonVertexIndex+infoObject.dataSize;for(let i=polygonVertexIndex,j=0;i<infoObject;i++,
j++)polygonIndex[j]=vertexIndex[i];return polygonIndex}function generateTransform(transformData){var lTranslationM=new _three.Matrix4;const lPreRotationM=new _three.Matrix4,lRotationM=new _three.Matrix4,lPostRotationM=new _three.Matrix4,lScalingM=new _three.Matrix4,lScalingPivotM=new _three.Matrix4,lScalingOffsetM=new _three.Matrix4,lRotationOffsetM=new _three.Matrix4,lRotationPivotM=new _three.Matrix4,lParentGX=new _three.Matrix4;var lParentLX=new _three.Matrix4;const lGlobalT=new _three.Matrix4;
...
transformData.rotationOffset&&lRotationOffsetM.setPosition(tempVec.fromArray(transformData.rotationOffset));transformData.rotationPivot&&lRotationPivotM.setPosition(tempVec.fromArray(transformData.rotationPivot));transformData.parentMatrixWorld&&(lParentLX.copy(transformData.parentMatrix),lParentGX.copy(transformData.parentMatrixWorld));array=lPreRotationM.clone().multiply(lRotationM).multiply(lPostRotationM);const lParentGRM=new _three.Matrix4;lParentGRM.extractRotation(lParentGX);transformData=
new _three.Matrix4;transformData.copyPosition(lParentGX);transformData=transformData.clone().invert().multiply(lParentGX);const lParentGSM=lParentGRM.clone().invert().multiply(transformData);transformData=new _three.Matrix4;0===inheritType?transformData.copy(lParentGRM).multiply(array).multiply(lParentGSM).multiply(lScalingM):1===inheritType?transformData.copy(lParentGRM).multiply(lParentGSM).multiply(array).multiply(lScalingM):(lParentLX=(new _three.Matrix4).scale((new _three.Vector3).setFromMatrixScale(lParentLX)).clone().invert(),
lParentLX=lParentGSM.clone().multiply(lParentLX),transformData.copy(lParentGRM).multiply(array).multiply(lParentLX).multiply(lScalingM));lParentLX=lRotationPivotM.clone().invert();inheritType=lScalingPivotM.clone().invert();lTranslationM=lTranslationM.clone().multiply(lRotationOffsetM).multiply(lRotationPivotM).multiply(lPreRotationM).multiply(lRotationM).multiply(lPostRotationM).multiply(lParentLX).multiply(lScalingOffsetM).multiply(lScalingPivotM).multiply(lScalingM).multiply(inheritType);lTranslationM=
(new _three.Matrix4).copyPosition(lTranslationM);lTranslationM=lParentGX.clone().multiply(lTranslationM);lGlobalT.copyPosition(lTranslationM);lTranslationM=lGlobalT.clone().multiply(transformData);lTranslationM.premultiply(lParentGX.invert());return lTranslationM}function getEulerOrder(order){order=order||0;const enums="ZYX YZX XZY ZXY YXZ XYZ".split(" ");return 6===order?(console.warn("THREE.FBXLoader: unsupported Euler Order: Spherical XYZ. Animations and rotations may be incorrect."),enums[0]):
enums[order]}function parseNumberArray(value){return value.split(",").map(function(val){return parseFloat(val)})}function convertArrayBufferToString(buffer,from,to){void 0===from&&(from=0);void 0===to&&(to=buffer.byteLength);return _three.LoaderUtils.decodeText(new Uint8Array(buffer,from,to))}function append(a,b){for(let i=0,j=a.length,l=b.length;i<l;i++,j++)a[j]=b[i]}function inject(a1,index,a2){return a1.slice(0,index).concat(a2).concat(a1.slice(index))}Object.defineProperty(exports,"__esModule",
{value:!0});exports.FBXLoader=void 0;var _three=require("module$node_modules$three$build$three_module"),_fflate=require("module$node_modules$fflate$esm$browser"),_NURBSCurve=require("module$node_modules$three_stdlib$curves$NURBSCurve");let fbxTree,connections,sceneGraph;class FBXLoader extends _three.Loader{constructor(manager){super(manager)}load(url,onLoad,onProgress,onError){const scope=this,path=""===scope.path?_three.LoaderUtils.extractUrlBase(url):scope.path,loader=new _three.FileLoader(this.manager);
loader.setPath(scope.path);loader.setResponseType("arraybuffer");loader.setRequestHeader(scope.requestHeader);loader.setWithCredentials(scope.withCredentials);loader.load(url,function(buffer){try{onLoad(scope.parse(buffer,path))}catch(e){onError?onError(e):console.error(e),scope.manager.itemError(url)}},onProgress,onError)}parse(FBXBuffer,path){if(21<=FBXBuffer.byteLength&&"Kaydara FBX Binary  \x00"===convertArrayBufferToString(FBXBuffer,0,21))fbxTree=(new BinaryParser).parse(FBXBuffer);else{FBXBuffer=
...
getFbxVersion(FBXBuffer));fbxTree=(new TextParser).parse(FBXBuffer)}path=(new _three.TextureLoader(this.manager)).setPath(this.resourcePath||path).setCrossOrigin(this.crossOrigin);return(new FBXTreeParser(path,this.manager)).parse(fbxTree)}}exports.FBXLoader=FBXLoader;class FBXTreeParser{constructor(textureLoader,manager){this.textureLoader=textureLoader;this.manager=manager}parse(){connections=this.parseConnections();var images=this.parseImages();images=this.parseTextures(images);images=this.parseMaterials(images);
const deformers=this.parseDeformers(),geometryMap=(new GeometryParser).parse(deformers);this.parseScene(deformers,geometryMap,images);return sceneGraph}parseConnections(){const connectionMap=new Map;"Connections"in fbxTree&&fbxTree.Connections.connections.forEach(function(rawConnection){var fromID=rawConnection[0];const toID=rawConnection[1];rawConnection=rawConnection[2];connectionMap.has(fromID)||connectionMap.set(fromID,{parents:[],children:[]});const parentRelationship={ID:toID,relationship:rawConnection};
connectionMap.get(fromID).parents.push(parentRelationship);connectionMap.has(toID)||connectionMap.set(toID,{parents:[],children:[]});fromID={ID:fromID,relationship:rawConnection};connectionMap.get(toID).children.push(fromID)});return connectionMap}parseImages(){const images={},blobs={};if("Video"in fbxTree.Objects){var videoNodes=fbxTree.Objects.Video;for(const nodeID in videoNodes){const videoNode=videoNodes[nodeID];var id$jscomp$0=parseInt(nodeID);images[id$jscomp$0]=videoNode.RelativeFilename||
videoNode.Filename;"Content"in videoNode&&(id$jscomp$0="string"===typeof videoNode.Content&&""!==videoNode.Content,videoNode.Content instanceof ArrayBuffer&&0<videoNode.Content.byteLength||id$jscomp$0)&&(id$jscomp$0=this.parseImage(videoNodes[nodeID]),blobs[videoNode.RelativeFilename||videoNode.Filename]=id$jscomp$0)}}for(const id in images)videoNodes=images[id],images[id]=void 0!==blobs[videoNodes]?blobs[videoNodes]:images[id].split("\\").pop();return images}parseImage(videoNode){var content=videoNode.Content;
videoNode=videoNode.RelativeFilename||videoNode.Filename;const extension=videoNode.slice(videoNode.lastIndexOf(".")+1).toLowerCase();switch(extension){case "bmp":videoNode="image/bmp";break;case "jpg":case "jpeg":videoNode="image/jpeg";break;case "png":videoNode="image/png";break;case "tif":videoNode="image/tiff";break;case "tga":null===this.manager.getHandler(".tga")&&console.warn("FBXLoader: TGA loader not found, skipping ",videoNode);videoNode="image/tga";break;default:console.warn('FBXLoader: Image type "'+
extension+'" is not supported.');return}if("string"===typeof content)return"data:"+videoNode+";base64,"+content;content=new Uint8Array(content);return window.URL.createObjectURL(new Blob([content],{type:videoNode}))}parseTextures(images){const textureMap=new Map;if("Texture"in fbxTree.Objects){const textureNodes=fbxTree.Objects.Texture;for(const nodeID in textureNodes){const texture=this.parseTexture(textureNodes[nodeID],images);textureMap.set(parseInt(nodeID),texture)}}return textureMap}parseTexture(textureNode,
images){images=this.loadTexture(textureNode,images);images.ID=textureNode.id;images.name=textureNode.attrName;const wrapModeU=textureNode.WrapModeU;var wrapModeV=textureNode.WrapModeV;wrapModeV=void 0!==wrapModeV?wrapModeV.value:0;images.wrapS=0===(void 0!==wrapModeU?wrapModeU.value:0)?_three.RepeatWrapping:_three.ClampToEdgeWrapping;images.wrapT=0===wrapModeV?_three.RepeatWrapping:_three.ClampToEdgeWrapping;"Scaling"in textureNode&&(textureNode=textureNode.Scaling.value,images.repeat.x=textureNode[0],
images.repeat.y=textureNode[1]);return images}loadTexture(textureNode,images){let fileName;const currentPath=this.textureLoader.path,children=connections.get(textureNode.id).children;void 0!==children&&0<children.length&&void 0!==images[children[0].ID]&&(fileName=images[children[0].ID],0!==fileName.indexOf("blob:")&&0!==fileName.indexOf("data:")||this.textureLoader.setPath(void 0));images=textureNode.FileName.slice(-3).toLowerCase();"tga"===images?(images=this.manager.getHandler(".tga"),null===images?
(console.warn("FBXLoader: TGA loader not found, creating placeholder texture for",textureNode.RelativeFilename),textureNode=new _three.Texture):(images.setPath(this.textureLoader.path),textureNode=images.load(fileName))):"psd"===images?(console.warn("FBXLoader: PSD textures are not supported, creating placeholder texture for",textureNode.RelativeFilename),textureNode=new _three.Texture):textureNode=this.textureLoader.load(fileName);this.textureLoader.setPath(currentPath);return textureNode}parseMaterials(textureMap){const materialMap=
new Map;if("Material"in fbxTree.Objects){const materialNodes=fbxTree.Objects.Material;for(const nodeID in materialNodes){const material=this.parseMaterial(materialNodes[nodeID],textureMap);null!==material&&materialMap.set(parseInt(nodeID),material)}}return materialMap}parseMaterial(materialNode,textureMap){const ID=materialNode.id,name=materialNode.attrName;var type=materialNode.ShadingModel;"object"===typeof type&&(type=type.value);if(!connections.has(ID))return null;materialNode=this.parseParameters(materialNode,
textureMap,ID);switch(type.toLowerCase()){case "phong":type=new _three.MeshPhongMaterial;break;case "lambert":type=new _three.MeshLambertMaterial;break;default:console.warn('THREE.FBXLoader: unknown material type "%s". Defaulting to MeshPhongMaterial.',type),type=new _three.MeshPhongMaterial}type.setValues(materialNode);type.name=name;return type}parseParameters(materialNode,textureMap,ID){const parameters={};materialNode.BumpFactor&&(parameters.bumpScale=materialNode.BumpFactor.value);materialNode.Diffuse?
parameters.color=(new _three.Color).fromArray(materialNode.Diffuse.value):!materialNode.DiffuseColor||"Color"!==materialNode.DiffuseColor.type&&"ColorRGB"!==materialNode.DiffuseColor.type||(parameters.color=(new _three.Color).fromArray(materialNode.DiffuseColor.value));materialNode.DisplacementFactor&&(parameters.displacementScale=materialNode.DisplacementFactor.value);materialNode.Emissive?parameters.emissive=(new _three.Color).fromArray(materialNode.Emissive.value):!materialNode.EmissiveColor||
"Color"!==materialNode.EmissiveColor.type&&"ColorRGB"!==materialNode.EmissiveColor.type||(parameters.emissive=(new _three.Color).fromArray(materialNode.EmissiveColor.value));materialNode.EmissiveFactor&&(parameters.emissiveIntensity=parseFloat(materialNode.EmissiveFactor.value));materialNode.Opacity&&(parameters.opacity=parseFloat(materialNode.Opacity.value));1>parameters.opacity&&(parameters.transparent=!0);materialNode.ReflectionFactor&&(parameters.reflectivity=materialNode.ReflectionFactor.value);
materialNode.Shininess&&(parameters.shininess=materialNode.Shininess.value);materialNode.Specular?parameters.specular=(new _three.Color).fromArray(materialNode.Specular.value):materialNode.SpecularColor&&"Color"===materialNode.SpecularColor.type&&(parameters.specular=(new _three.Color).fromArray(materialNode.SpecularColor.value));const scope=this;connections.get(ID).children.forEach(function(child){const type=child.relationship;switch(type){case "Bump":parameters.bumpMap=scope.getTexture(textureMap,
child.ID);break;case "Maya|TEX_ao_map":parameters.aoMap=scope.getTexture(textureMap,child.ID);break;case "DiffuseColor":case "Maya|TEX_color_map":parameters.map=scope.getTexture(textureMap,child.ID);void 0!==parameters.map&&(parameters.map.encoding=_three.sRGBEncoding);break;case "DisplacementColor":parameters.displacementMap=scope.getTexture(textureMap,child.ID);break;case "EmissiveColor":parameters.emissiveMap=scope.getTexture(textureMap,child.ID);void 0!==parameters.emissiveMap&&(parameters.emissiveMap.encoding=
_three.sRGBEncoding);break;case "NormalMap":case "Maya|TEX_normal_map":parameters.normalMap=scope.getTexture(textureMap,child.ID);break;case "ReflectionColor":parameters.envMap=scope.getTexture(textureMap,child.ID);void 0!==parameters.envMap&&(parameters.envMap.mapping=_three.EquirectangularReflectionMapping,parameters.envMap.encoding=_three.sRGBEncoding);break;case "SpecularColor":parameters.specularMap=scope.getTexture(textureMap,child.ID);void 0!==parameters.specularMap&&(parameters.specularMap.encoding=
_three.sRGBEncoding);break;case "TransparentColor":case "TransparencyFactor":parameters.alphaMap=scope.getTexture(textureMap,child.ID);parameters.transparent=!0;break;default:console.warn("THREE.FBXLoader: %s map is not supported in three.js, skipping texture.",type)}});return parameters}getTexture(textureMap,id){"LayeredTexture"in fbxTree.Objects&&id in fbxTree.Objects.LayeredTexture&&(console.warn("THREE.FBXLoader: layered textures are not supported in three.js. Discarding all but first layer."),
...
return geo}parseGeoNode(geoNode,skeleton){const geoInfo={};geoInfo.vertexPositions=void 0!==geoNode.Vertices?geoNode.Vertices.a:[];geoInfo.vertexIndices=void 0!==geoNode.PolygonVertexIndex?geoNode.PolygonVertexIndex.a:[];geoNode.LayerElementColor&&(geoInfo.color=this.parseVertexColors(geoNode.LayerElementColor[0]));geoNode.LayerElementMaterial&&(geoInfo.material=this.parseMaterialIndices(geoNode.LayerElementMaterial[0]));geoNode.LayerElementNormal&&(geoInfo.normal=this.parseNormals(geoNode.LayerElementNormal[0]));
if(geoNode.LayerElementUV){geoInfo.uv=[];let i=0;for(;geoNode.LayerElementUV[i];)geoNode.LayerElementUV[i].UV&&geoInfo.uv.push(this.parseUVs(geoNode.LayerElementUV[i])),i++}geoInfo.weightTable={};null!==skeleton&&(geoInfo.skeleton=skeleton,skeleton.rawBones.forEach(function(rawBone,i){rawBone.indices.forEach(function(index,j){void 0===geoInfo.weightTable[index]&&(geoInfo.weightTable[index]=[]);geoInfo.weightTable[index].push({id:i,weight:rawBone.weights[j]})})}));return geoInfo}genBuffers(geoInfo){const buffers=
{vertex:[],normal:[],colors:[],uvs:[],materialIndex:[],vertexWeights:[],weightsIndices:[]};let polygonIndex=0,faceLength=0,displayedWeightsWarning=!1,facePositionIndexes=[],faceNormals=[],faceColors=[],faceUVs=[],faceWeights=[],faceWeightIndices=[];const scope=this;geoInfo.vertexIndices.forEach(function(vertexIndex,polygonVertexIndex){let materialIndex,endOfFace=!1;0>vertexIndex&&(vertexIndex^=-1,endOfFace=!0);let weightIndices=[],weights=[];facePositionIndexes.push(3*vertexIndex,3*vertexIndex+1,
3*vertexIndex+2);if(geoInfo.color){var data=getData(polygonVertexIndex,polygonIndex,vertexIndex,geoInfo.color);faceColors.push(data[0],data[1],data[2])}if(geoInfo.skeleton){void 0!==geoInfo.weightTable[vertexIndex]&&geoInfo.weightTable[vertexIndex].forEach(function(wt){weights.push(wt.weight);weightIndices.push(wt.id)});if(4<weights.length){displayedWeightsWarning||(console.warn("THREE.FBXLoader: Vertex has more than 4 skinning weights assigned to vertex. Deleting additional weights."),displayedWeightsWarning=
!0);const wIndex=[0,0,0,0],Weight=[0,0,0,0];weights.forEach(function(weight,weightIndex){let currentWeight=weight,currentIndex=weightIndices[weightIndex];Weight.forEach(function(comparedWeight,comparedWeightIndex,comparedWeightArray){currentWeight>comparedWeight&&(comparedWeightArray[comparedWeightIndex]=currentWeight,currentWeight=comparedWeight,comparedWeight=wIndex[comparedWeightIndex],wIndex[comparedWeightIndex]=currentIndex,currentIndex=comparedWeight)})});weightIndices=wIndex;weights=Weight}for(;4>
weights.length;)weights.push(0),weightIndices.push(0);for(data=0;4>data;++data)faceWeights.push(weights[data]),faceWeightIndices.push(weightIndices[data])}geoInfo.normal&&(data=getData(polygonVertexIndex,polygonIndex,vertexIndex,geoInfo.normal),faceNormals.push(data[0],data[1],data[2]));geoInfo.material&&"AllSame"!==geoInfo.material.mappingType&&(materialIndex=getData(polygonVertexIndex,polygonIndex,vertexIndex,geoInfo.material)[0]);geoInfo.uv&&geoInfo.uv.forEach(function(uv,i){uv=getData(polygonVertexIndex,
...
(i-1)+2]]),buffers.vertex.push(geoInfo.vertexPositions[facePositionIndexes[3*i]]),buffers.vertex.push(geoInfo.vertexPositions[facePositionIndexes[3*i+1]]),buffers.vertex.push(geoInfo.vertexPositions[facePositionIndexes[3*i+2]]),geoInfo.skeleton&&(buffers.vertexWeights.push(faceWeights[0]),buffers.vertexWeights.push(faceWeights[1]),buffers.vertexWeights.push(faceWeights[2]),buffers.vertexWeights.push(faceWeights[3]),buffers.vertexWeights.push(faceWeights[4*(i-1)]),buffers.vertexWeights.push(faceWeights[4*
(i-1)+1]),buffers.vertexWeights.push(faceWeights[4*(i-1)+2]),buffers.vertexWeights.push(faceWeights[4*(i-1)+3]),buffers.vertexWeights.push(faceWeights[4*i]),buffers.vertexWeights.push(faceWeights[4*i+1]),buffers.vertexWeights.push(faceWeights[4*i+2]),buffers.vertexWeights.push(faceWeights[4*i+3]),buffers.weightsIndices.push(faceWeightIndices[0]),buffers.weightsIndices.push(faceWeightIndices[1]),buffers.weightsIndices.push(faceWeightIndices[2]),buffers.weightsIndices.push(faceWeightIndices[3]),buffers.weightsIndices.push(faceWeightIndices[4*
...
type);}}}class BinaryReader{constructor(buffer,littleEndian){this.dv=new DataView(buffer);this.offset=0;this.littleEndian=void 0!==littleEndian?littleEndian:!0}getOffset(){return this.offset}size(){return this.dv.buffer.byteLength}skip(length){this.offset+=length}getBoolean(){return 1===(this.getUint8()&1)}getBooleanArray(size){const a=[];for(let i=0;i<size;i++)a.push(this.getBoolean());return a}getUint8(){const value=this.dv.getUint8(this.offset);this.offset+=1;return value}getInt16(){const value=
this.dv.getInt16(this.offset,this.littleEndian);this.offset+=2;return value}getInt32(){const value=this.dv.getInt32(this.offset,this.littleEndian);this.offset+=4;return value}getInt32Array(size){const a=[];for(let i=0;i<size;i++)a.push(this.getInt32());return a}getUint32(){const value=this.dv.getUint32(this.offset,this.littleEndian);this.offset+=4;return value}getInt64(){let low,high;this.littleEndian?(low=this.getUint32(),high=this.getUint32()):(high=this.getUint32(),low=this.getUint32());return high&
2147483648?(high=~high&4294967295,low=~low&4294967295,4294967295===low&&(high=high+1&4294967295),-(4294967296*high+(low+1&4294967295))):4294967296*high+low}getInt64Array(size){const a=[];for(let i=0;i<size;i++)a.push(this.getInt64());return a}getUint64(){let low,high;this.littleEndian?(low=this.getUint32(),high=this.getUint32()):(high=this.getUint32(),low=this.getUint32());return 4294967296*high+low}getFloat32(){const value=this.dv.getFloat32(this.offset,this.littleEndian);this.offset+=4;return value}getFloat32Array(size){const a=
[];for(let i=0;i<size;i++)a.push(this.getFloat32());return a}getFloat64(){const value=this.dv.getFloat64(this.offset,this.littleEndian);this.offset+=8;return value}getFloat64Array(size){const a=[];for(let i=0;i<size;i++)a.push(this.getFloat64());return a}getArrayBuffer(size){const value=this.dv.buffer.slice(this.offset,this.offset+size);this.offset+=size;return value}getString(size){let a=[];for(let i=0;i<size;i++)a[i]=this.getUint8();size=a.indexOf(0);0<=size&&(a=a.slice(0,size));return _three.LoaderUtils.decodeText(new Uint8Array(a))}}
class FBXTree{add(key,val){this[key]=val}}const dataArray=[],tempEuler=new _three.Euler,tempVec=new _three.Vector3}
//# sourceMappingURL=module$node_modules$three_stdlib$loaders$FBXLoader.js.map

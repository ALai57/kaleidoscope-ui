...
import { getAdminHost, getAdminRole, getWriterRole, isSiteAdmin, isWriter } from './authHelpers';
...
describe('getWriterRole', () => {
  afterEach(() => {
    vi.unstubAllGlobals();
  });

  it('appends the writer suffix to the current host', () => {
    vi.stubGlobal('window', {
      location: { host: 'andrewslai.com', hostname: 'andrewslai.com' },
    });
    expect(getWriterRole()).toBe('andrewslai.com:writer');
  });
});
...
describe('isWriter', () => {
  ...
  it('is true when the user holds the writer role for the current host', () => {
    stubHost('andrewslai.com');
    expect(isWriter({ realm_access: { roles: ['andrewslai.com:writer'] } })).toBe(true);
  });

  it('is true for a site admin (admins are writers)', () => {
    stubHost('andrewslai.com');
    expect(isWriter({ realm_access: { roles: ['andrewslai.com:admin'] } })).toBe(true);
  });

  it("is false when the user holds only another host's writer role", () => {
    stubHost('andrewslai.com');
    expect(isWriter({ realm_access: { roles: ['caheriaguilar.com:writer'] } })).toBe(false);
  });

  it('is false for a user with no roles', () => {
    stubHost('andrewslai.com');
    expect(isWriter({ realm_access: { roles: [] } })).toBe(false);
  });
